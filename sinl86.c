...
int i;
     ...
     if (compare(unit, ALT_rezword[RW_PROGR])) { /*  по-русски? - инициализируем русским */
		 Flag_NAMES = OFF;		/* флаг генерации Си-имен из имен процессов и переменных для русского варианта невозможен */
         ...
         if (compare(unit, ENG_rezword[RW_PROGR])) { /* слово Прогр по английски? */
			  ...
			  for (i = 0; i < ERRMSGNUM; i++) {
				  strncpy(error_array[i], ENG_error_array[i], ERRMSGLEN);
			  }
...
INT16S Statement_Body (void)
{
  ...
  !!!            switch (Start_Body()) {  /* START PROC имя процесса ; */
              ...
              break;
         case RW_SET:	/* 1.1 */
 !!!             switch (Set_Body()) {    /* SET (NEXT | STATE имя состояния) ; */
                   case ERROR:         /* Error */
                        return(ERROR);
                   case ZERO:          /* EOF */
                        return(ZERO);
              ...
              break;
		 case RW_RESET:
			 switch (Reset_Body()) {  /* RESET TIMEOUT; */
			 case ERROR:         /* Error */
				 return(ERROR);
			 case ZERO:          /* EOF */
				 return(ZERO);
...
!!!!			  AddNewProcInUse(unit); /* add new proc name in the list of process in use */
              if (!select_unit()) return(ZERO);   /* ; */
              if (!compare(unit, rezword[RW_SEMICOLON])) {
                   error_msg(10, line_count, unit, NONE);
                   return(ERROR);
...
/********************************************
* обработка описания RESET
*  ВХОД - после обнаружения рез.слова RESET
*  unit  V
*  RESET TIMEOUT ; 
*  RETURN: EOF   (обнаружен конец файла)
*          OK    (обработано без ошибки)
...
int    Reset_Body (void)
{
	...
	case RW_TIMEOUT:                           /* TIMEOUT */
		...
		break;
	default:                 /*  иначе - ошибка RESET TIMEOUT */
		error_msg(80, line_count, unit, NONE);
		return(ERROR);
	} /* end of switch */
	return(!ZERO);
};
...
/********************************************
* обработка описания RESTART
*  ВХОД - после обнаружения рез.слова RESTART
*  unit  V
*  RESET ;
*  RETURN: EOF   (обнаружен конец файла)
*          OK    (обработано без ошибки)
...
int    Set_Body (void)
{
    ...
    switch (compare_RW(unit)) {
         case RW_STATE:                           /* СОСТ */
			  ...
			  AddNewStateInUse(unit); /* add new state name in the list of states in use */
              if (!select_unit()) return(ZERO);   /* ; */
              if (!compare(unit, rezword[RW_SEMICOLON])) {
                   error_msg(10, line_count, unit, NONE);
                   return(ERROR);
