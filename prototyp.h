...
void GenResetTimeoutForCurrentProc(void); /* Генерация сброса таймаута для т-процесса */
...
void Sem_Set_Body(void);            /* обработка описания перехода УСТ (SET) СОСТ */
void Sem_Reset_Body(void);          /* обработка описания RESET TIMEOUT */
void Sem_Restart_Body(void);          /* обработка описания СТАРТ */
...
int    Set_Body(void);            /* обработка описания УСТ(SET) СОСТ */
