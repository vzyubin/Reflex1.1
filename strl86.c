					...
					* to port bits tree ===*/
{
	...
	char   name_port[NAME_LENGTH];            /* For Port Name */
...
{
	char  name[NAME_LENGTH];
	struct proc_in_use *before;
	struct proc_in_use *next;
...
struct disc_process	/*=== For processes tree ===*/
{
	INT16S logname;
	char  name[NAME_LENGTH];
	INT16S FlagReachability;
	struct proc_in_use *proc; /* List of Used Processes*/
...
{
	char  name[NAME_LENGTH];
	struct state_in_use *before;
	struct state_in_use *next;
...
struct disc_state	/*=== For states tree ===*/
{
	...
	char  name[NAME_LENGTH];
	INT16S FlagReachability;
	struct state_in_use *state; /* List of Used Processes*/
...
Con_Value(char  *buf){
	if (!Flag_NAMES)
		...
		sprintf(buf, "%s", Current_Constant->name);
	return(!ZERO);
};
	...
	char  *unit_name;
{
	...
	if(Current_Process == ZERO){
		...
		Current_Process->FlagReachability = ON; /* the first process is always reachable */
		Current_Process->proc = ZERO; /* init list of used processes */
		...
		if(Current_Process->next==ZERO){
			printf("\n%RCSL-XX-FATERR, No enough memory to compile");
			exit(ZERO);
		}
		...
		Current_Process->FlagReachability = OFF; /* init  reachability flag */
		Current_Process->proc = ZERO; /* init list of used processes */
		Current_Process->next = ZERO;
	}
	for(i=0;i<(NAME_LENGTH-1);i++){
...
/* Add new state name to the list of state in use for current state */

AddNewProcInUse(unit) /* add new proc name to the list of processes in use */
char  *unit;
{
	struct proc_in_use * ProcList;
	int i;
	if (Current_Process->proc == ZERO) { /* first process in use */
		Current_Process->proc = (struct proc_in_use *)
			calloc(1, sizeof(struct proc_in_use));
		if (Current_Process->proc == ZERO){
			!printf("\n Not Enought Space Exist");
			exit(ZERO);
		}
		strncpy((Current_Process->proc)->name, unit, (NAME_LENGTH - 1));
		(Current_Process->proc)->before = ZERO;
		(Current_Process->proc)->next = ZERO;
	}
	else {
		ProcList = Current_Process->proc;
		while (ProcList->next != ZERO) { // find the end
			ProcList = ProcList->next;
		}
		ProcList->next = (struct proc_in_use *)
			calloc(1, sizeof(struct proc_in_use));
		if (ProcList->next == ZERO){
			!printf("\n Not Enought Space Exist");
			exit(ZERO);
		}
		(ProcList->next)->next = ZERO; /* init next for new state */
		(ProcList->next)->before = ProcList; /* init before for new state */
		ProcList = ProcList->next;
		strncpy(ProcList->name, unit, (NAME_LENGTH - 1)); /* init name for new state in use */
	}
}
	...
	char  *unit_name;
{
	...
	!		printf("\n Not Enought Space Exist");
		...
		Current_State->FlagReachability = ON; /* the first state is always reachable */
		Current_State->state = ZERO; /* init list of used states */
		...
		Current_State->FlagReachability = OFF; /* init reachability flag */
		Current_State->state = ZERO; /* init list of used states */
		Current_State->next = ZERO;
	}
	for (i = 0; i < (NAME_LENGTH - 1); i++) {
...
AddNewStateInUse (unit) /* add new state name ti the list of states in use */
char  *unit;
{
	struct state_in_use * StateList;
	int i;
	if (Current_State->state == ZERO) { /* first state in use */
		Current_State->state = (struct state_in_use *)
			calloc(1, sizeof(struct state_in_use));
		if (Current_State->state == ZERO){
			!printf("\n Not Enought Space Exist");
			exit(ZERO);
		}
		strncpy((Current_State->state)->name, unit, (NAME_LENGTH - 1));
		(Current_State->state)->before = ZERO;
		(Current_State->state)->next = ZERO;		
	}
	else {
		StateList = Current_State->state;
		while (StateList->next != ZERO) { // find the end
			StateList = StateList->next;
		}
		StateList->next = (struct state_in_use *)
			calloc(1, sizeof(struct state_in_use));
		if (StateList->next == ZERO){
			!printf("\n Not Enought Space Exist");
			exit(ZERO);
		}
		(StateList->next)->next = ZERO; /* init next for new state */
		(StateList->next)->before = StateList; /* init before for new state */ 
		StateList = StateList->next;
		strncpy(StateList->name, unit, (NAME_LENGTH - 1)); /* init name for new state in use */
	}
}
...
int InitVariable (char *port_name, int value)
{
	...
	if (!Check_Name_Input_Group(port_name)) {  /* ЕСЛИ ПОРТ - ВХОДНОЙ */
		...
		strncpy(Current_Part_Of_Var->name_port, Current_Input_Group->name, (NAME_LENGTH - 1));
	...
	if (!Check_Name_RAM_Group(port_name)) {           /* ЕСЛИ ПОРТ - ОЗУ */
		...
		strncpy(Current_Part_Of_Var->name_port, Current_RAM_Group->name, (NAME_LENGTH - 1));
	...
	if (!Check_Name_Output_Group(port_name)) {   /* ЕСЛИ ПОРТ ВЫХОДНОЙ */
		...
		strncpy(Current_Part_Of_Var->name_port, Current_Output_Group->name, (NAME_LENGTH - 1));
	...
	*/
	...
	if (Current_Variable->forproc == (struct disc_forproc *)RW_ALL) { /* для всех */
		if (!Flag_NAMES)
			...
			sprintf(Variable_In_Current_Process->name_from_proc, "%s_%s", /* инициируем имя */
				Current_Process->name,
				Current_Variable->name);
...
void GenDefConstant (char *unit)
{
	...
	if (!Flag_NAMES)
		...
		sprintf(string, "#define %s ", Current_Constant->name, Current_Constant->name);
	fputs(string, fptr_C);
	...
	if (!Flag_NAMES)
		...
		sprintf(string, "%s - %s", Current_Constant->name, Current_Constant->name);
...
void GenDgt (char *digit)
{
	...
	if(!Check_Name_Constant(digit))
		if (!Flag_NAMES)
			...
			sprintf(string, "%s%s", spaces, Current_Constant->name);
	else
...
void Gen_Dgt (char *digit)
{
	...
	if(!Check_Name_Constant(digit))
		if (!Flag_NAMES)
			...
			sprintf(string, "%s", Current_Constant->name);
	else
			...
			if (Current_Variable->type != RW_FROM) { /* если описана не в текущем процессе - пропускаем */
				...
				case RW_SHORT:
					if (!Flag_NAMES)
						...
						sprintf(string, "char %s_%s%s", Current_Process->name, Current_Variable->name, var_IO_type);
					break;
				case RW_INT:
					if (!Flag_NAMES)
						sprintf(string, "int P%dV%d%s", Current_Process->logname, Current_Variable->logname, var_IO_type);
					else
						sprintf(string, "int %s_%s%s", Current_Process->name, Current_Variable->name, var_IO_type);
					break;
				case RW_LONG:
					if (!Flag_NAMES)
						sprintf(string, "long P%dV%d%s", Current_Process->logname, Current_Variable->logname, var_IO_type);
					else
						sprintf(string, "long %s_%s%s", Current_Process->name, Current_Variable->name, var_IO_type);
					break;
				case RW_FLOAT:
					if (!Flag_NAMES)
						sprintf(string, "float P%dV%d%s", Current_Process->logname, Current_Variable->logname, var_IO_type);
					else
						sprintf(string, "float %s_%s%s", Current_Process->name, Current_Variable->name, var_IO_type);
					break;
				case RW_DOUBLE:
					if (!Flag_NAMES)
						sprintf(string, "double P%dV%d%s", Current_Process->logname, Current_Variable->logname, var_IO_type);
					else
						sprintf(string, "double %s_%s%s", Current_Process->name, Current_Variable->name, var_IO_type);
					break;
				default:
					/* !!!!!!!!!!!!!!!!! */
					break;
		...
		for (; Current_Output_Group->logname != NOT_INITIALIZE; ) {
			...
			if (Current_Output_Group->size == 8) {
				if (!Flag_NAMES)
					...
					sprintf(string, "char  %s, F_%s;\n",
					Current_Output_Group->name,
					Current_Output_Group->name);
			}
			else {
				if (!Flag_NAMES)
					sprintf(string, "int PO%d;\ncharFPO%d;\n",
					Current_Output_Group->logname,
					Current_Output_Group->logname);
				else
					sprintf(string, "int %s;\nchar F_%s;\n",
		...
		for (; Current_Input_Group->logname != NOT_INITIALIZE; ) {
			if (Current_Input_Group->size == 8) {
				if (!Flag_NAMES)
					...
					sprintf(string, "\t%s = INBYTE((INT16U)%s, (INT16U)%s); /* чтение байта */\n",
					Current_Input_Group->name,
					Current_Input_Group->MB_address,
					Current_Input_Group->address);
				...
				if (!Flag_NAMES)
					...
					sprintf(string, "\t%s = INWORD((INT16U)%s, (INT16U)%s); /* чтение слова */\n",
					Current_Input_Group->name,
					Current_Input_Group->MB_address,
					Current_Input_Group->address);
			}
		...
		for (; Current_Output_Group->logname != NOT_INITIALIZE; ) {
			if (Current_Output_Group->size == 8) {
				if (!Flag_NAMES)
					...
					sprintf(string, "\tF_%s = OFF; %s = INBYTE((INT16U)%s, (INT16U)%s);\n",
					Current_Output_Group->name,
					Current_Output_Group->name,
					Current_Output_Group->MB_address,
					Current_Output_Group->address);
				...
				if (!Flag_NAMES)
					...
					sprintf(string, "\tF_%s = OFF; %s = INWORD((INT16U)%s, (INT16U)%s);\n",
					Current_Output_Group->name,
					Current_Output_Group->name,
					Current_Output_Group->MB_address,
					Current_Output_Group->address);
		...
		for (; Current_RAM_Group->logname != NOT_INITIALIZE; ) {
			if (Current_RAM_Group->size == 8) {
				if (!Flag_NAMES)
					...
					sprintf(string, "\tF_%s = OFF; %s = IN_RAM_BYTE(%s, %s, %s);\n",
					Current_RAM_Group->name,
					Current_RAM_Group->name,
					Current_RAM_Group->segment,
					Current_RAM_Group->offset,
				...
				if (!Flag_NAMES)
					...
					sprintf(string, "\tF_%s = OFF; %s = IN_RAM_WORD(%s, %s, %s);\n",
					Current_RAM_Group->name,
					Current_RAM_Group->name,
					Current_RAM_Group->segment,
					Current_RAM_Group->offset,
...
void Gen_Input_Variable (void)
{
	...
	case RW_LOG: /* for input logical registr type */
		if (!Flag_NAMES)
			...
			sprintf(string,
			"\tif (%s & MASK%d_S%d) %s_%s = 1;\n\telse %s_%s = 0;\n",
!			Current_Part_Of_Var->name_port,
			Current_Part_Of_Var->start_bit,
			Current_Part_Of_Var->size_port,
			Current_Process->name,
			Current_Variable->name,
		...
		if (Flag_TEST)
		{
			if (!Flag_NAMES)
				...
				sprintf(string_for_test, "INPUT LOG %s /*name*/ %s /*owner_input_port*/ %d /*start_bit*/  \n",
				Current_Variable->name,
				Current_Part_Of_Var->name_port,
				Current_Part_Of_Var->start_bit - 1);
	...
	case RW_INT:
		if (Current_Variable->number_of_busy_bit ==
			...
			if (!Flag_NAMES)
				...
				sprintf(string, "\t%s_%s = (INT16S)%s;\n", Current_Process->name, Current_Variable->name, Current_Part_Of_Var->name_port);
			fputs(string, fptr_INP);
		} else {
			...
			while ((Current_Part_Of_Var->start_bit + cur_part_bit) <
				(Current_Part_Of_Var->finish_bit)) {
					if (!Flag_NAMES)
						...
						sprintf(string,
						"\t%s_%s = 0;\n\tif (%s & MASK%d_S%d) %s_%s |= MASK%d_S16;\n",
						Current_Process->name,
						Current_Variable->name,
						Current_Part_Of_Var->name_port,
						(Current_Part_Of_Var->start_bit + cur_part_bit),
		...
		if (Flag_TEST)
		{
			if (!Flag_NAMES)
				...
				sprintf(string_for_test, "INPUT INT %s /*name*/ %s /*owner input port*/ %d /*start bit*/\n",
				Current_Variable->name,
				Current_Process->name,
				Current_Part_Of_Var->start_bit - 1);
			fputs(string_for_test, fptr_VARIABLES_FOR_TEST);
...
void Gen_Output_Variable (void)
{
	...
	case RW_LOG:
		if (!Flag_NAMES)
			...
			sprintf(string, "\tif (%s & MASK%d_S%d) %s_%s[0] = 1;\n\telse %s_%s[0] = 0;\n\t%s_%s[1] = %s_%s[0];\n",
			Current_Part_Of_Var->name_port,
			Current_Part_Of_Var->start_bit,
			Current_Part_Of_Var->size_port,
			Current_Process->name,
		...
		if (Flag_TEST)
		{
			if (!Flag_NAMES)
				...
				sprintf(string_for_test, "OUTPUT LOG %s /*name*/ %s /*owner_output_port*/ %d /*start_bit*/  \n",
				Current_Variable->name,
				Current_Part_Of_Var->name_port,
				Current_Part_Of_Var->start_bit - 1);
			fputs(string_for_test, fptr_VARIABLES_FOR_TEST);
		...
		if (Current_Variable->number_of_busy_bit ==
			...
			while ((Current_Part_Of_Var->start_bit + cur_part_bit) <
				...
				if (!Flag_NAMES)
					...
					sprintf(string,
					"\t%s_%s[0] = 0;\n\tif (%s & MASK%d_S%d) %s_%s[0] |= MASK%d_S16;\n\t %s_%s[1] = %s_%s[0];",
					Current_Process->name,
					Current_Variable->name,
					Current_Part_Of_Var->name_port,
					(Current_Part_Of_Var->start_bit + cur_part_bit),
				...
				fputs(string, fptr_INP);
					if (!Flag_NAMES)
						...
						sprintf(string,
						"\tif (%s_%s[0] != %s_%s[1]) {\n\t\tF_%s = ON;\n\t\tif (%s_%s[1] & MASK%d_S16) %s |= MASK%d_S%d;\n\t\telse %s &= ~MASK%d_S%d;\n\t}\n",
						Current_Process->name,
						Current_Variable->name,
						Current_Process->name,
						Current_Variable->name,
		...
		for (; Current_Output_Group->logname != NOT_INITIALIZE; ) {
			if (Current_Output_Group->size == 8) {
				if (!Flag_NAMES)
					...
					sprintf(string,
					"\tif(F_%s == ON) OUTBYTE((INT16U)%s, (INT16U)%s, (INT8U)~%s);\n",
					Current_Output_Group->name,
					Current_Output_Group->MB_address,
					Current_Output_Group->address,
					Current_Output_Group->name);
				...
				if (!Flag_NAMES)
					...
					sprintf(string,
					"\tif(F_%s == ON) OUTWORD((INT16U)%s, (INT16U)%s, (INT16U)~%s);\n",
					Current_Output_Group->name,
					Current_Output_Group->MB_address,
					Current_Output_Group->address,
					Current_Output_Group->name);
		...
		for(; Current_RAM_Group->logname != NOT_INITIALIZE; ) {
			if (Current_RAM_Group->size == 8) {
				if (!Flag_NAMES)
					...
					sprintf(string,
					"\tif (F_%s == ON) OUT_RAM_BYTE(%s, %s, %s, %s);\n",
					Current_RAM_Group->name,
					Current_RAM_Group->name,
					Current_RAM_Group->segment,
					Current_RAM_Group->offset,
				...
				if (!Flag_NAMES)
					...
					sprintf(string,
					"\tif (F_%s == ON) OUT_RAM_WORD(%s, %s, %s, %s);\n",
					Current_RAM_Group->name,
					Current_RAM_Group->name,
					Current_RAM_Group->segment,
					Current_RAM_Group->offset,
...
int GenStop (char  *unit)
{
	...
	sprintf(string, "Set_Stop(%d); /* stop proc %s */",
		Current_Process->logname,
		Current_Process->name);
	fputs(string, fptr_C);
	Current_Process = Copy_Process;
...
void GenStopForCurrentProc (void)
{
	...
	sprintf(string, "Set_Stop(%d); /* stop current proc %s */",
		Current_Process->logname,
		Current_Process->name);
	fputs(string, fptr_C);
	return;
...
int GenError (char  *unit)
{
	...
	sprintf(string, "Set_Error(%d); /* set error proc %s */",
		Current_Process->logname,
		Current_Process->name);
	fputs(string, fptr_C);
	Current_Process = Copy_Process;
...
void GenErrorForCurrentProc (void)
{
	...
	sprintf(string, "Set_Error(%d); /* set error current proc %s */",
		Current_Process->logname,
		Current_Process->name);
	fputs(string, fptr_C);
	return;
...
int GenStart (char  *unit)
{
	...
	sprintf(string, "Set_Start(%d); /* start proc %s */",
		Current_Process->logname,
		Current_Process->name);
	fputs(string, fptr_C);
	Current_Process = Copy_Process;
...
void GenStartForCurrentProc (void)
{
	...
	sprintf(string, "Set_Start(%d); /* start current proc %s */",
		Current_Process->logname,
		Current_Process->name);
	fputs(string, fptr_C);
	return;
...
/*****************************************
*    Генерация сброса таймаута текущего процесса
*****************************************/
void GenResetTimeoutForCurrentProc(void)
{
	char string[BUFFER_LENGTH];

	sprintf(string, "Reset_Timeout(%d);",
		Current_Process->logname);
	fputs(string, fptr_C);
	return;
};
...
/* 
/* /*================ Set State Of Process ===================*/ /* 1.1 */
/* void Reset_Timeout(INT16U pr_num)
/* {
...
/*****************************************
*    Генерация рестарта текущего процесса
*****************************************/
void GenRestartForCurrentProc(void)
{
	char string[BUFFER_LENGTH];

	sprintf(string, "Restart(%d);",
		Current_Process->logname);
	fputs(string, fptr_C);
	return;
};
...
/*
/* /*================ Restart Of Process ===================*/ /* 1.1 */
/* void Restart(INT16U pr_num)
/* {
...
int GenJMP (char *unit)
{
	...
	sprintf(string, "Set_State(%d, %d); /* set state %s */",
		Current_Process->logname,
		Current_State->logname,
		Current_State->name);
	fputs(string, fptr_C);
...
int GenSituationProcInSTOP (char *proc_name)  /* ситуация ПРОЦ в СТОП */
{
	...
	sprintf(string, "(Check_State(%d /* %s */) == STATE_OF_STOP)", 
		Current_Process->logname,
		Current_Process->name);
	fputs(string, fptr_C);
	Current_Process = Copy_Process;
...
int GenSituationProcInERROR (char *proc_name)
{
	...
	sprintf(string, "(Check_State(%d /* %s */) == STATE_OF_ERROR)", 
		Current_Process->logname,
		Current_Process->name);
	fputs(string, fptr_C);
	Current_Process = Copy_Process;
...
int GenSituationProcInACTIVE (char *proc_name)
{
	...
	sprintf(string, "(!(Check_State(%d /* %s */) & MASK_OF_INACTIVITY))", 
		Current_Process->logname,
		Current_Process->name);
	fputs(string, fptr_C);
	Current_Process = Copy_Process;
...
* <имя состояния>
* ! эта команда нарушает инкапсуляцию. Плохая команда. 
******************************************/
int GenSituationProcInState(char *proc_name, char *state_name)
{
	...
	sprintf(string, "(Check_State(%d /* %s */) == %d /* %s */)",
		Current_Process->logname,
		Current_Process->name,
		Current_State->logname,
		Current_State->name);
...
int GenTimeOut (char *num)
{
	...
	sprintf(string, "if (Timeout(%d /* %s */, %s)) ",
		Current_Process->logname,
		Current_Process->name,
		num);
	fputs(string, fptr_C);
	...
	/* return: 0 - Not Exist, 1-O'K and unit=Pc%d) */
{
	...
	if(!Check_Name_Output_Group(unit)){ /* if find */
		if (!Flag_NAMES)
			...
			sprintf(unit, "%s", Current_Output_Group->name);
		return(!ZERO);
	}
	if(!Check_Name_Input_Group(unit)){ /* if find */
		if (!Flag_NAMES)
			...
			sprintf(unit, "%s", Current_Input_Group->name);
		return(!ZERO);
	}
	if(!Check_Name_RAM_Group(unit)){ /* if find */
		if (!Flag_NAMES)
			...
			sprintf(unit, "%s", Current_RAM_Group->logname);
		return(!ZERO);
	}
	return(ZERO);
};
...
H_C_Constant(){
	char string[BUFFER_LENGTH];
	sprintf(string,
		"\n ИМЯ КОНСТАНТЫCONSTANT NAME: %s,  VALUE: %s,    L-NAME: C_%d",
		Current_Constant->name,
		Current_Constant->value,
	...
	if (Current_Variable != ZERO) {
		for (;;) {
			if (Current_Variable->type != RW_FROM) {
				...
				switch (Current_Variable->type){
				case RW_INT:
					fputs("\n TYPE: Integer",fptr_SYM);
					break;
				case RW_LOG:
					fputs("\n TYPE: Logical",fptr_SYM);
					break;
				case RW_LONG:
					fputs("\n TYPE: Long Integer",fptr_SYM);
					break;
				case RW_SHORT:
					fputs("\n TYPE: Short Integer",fptr_SYM);
					break;
				default:
					fputs("\n TYPE: Float",fptr_SYM);
					break;
	...
	if ((Current_Part_Of_Var != (struct disc_port *)ZERO) &&
		(Current_Part_Of_Var != (struct disc_port *)1)) {
			if (Current_Variable->type == RW_LOG) {
				fputs("\n\tmapped to bit ", fptr_SYM);
				sprintf(string, "%d port %s",
					(Current_Part_Of_Var->start_bit-1),
					Current_Part_Of_Var->name);
				fputs(string,fptr_SYM);
			} else {
				fputs("\n--------------------------------------------",fptr_SYM);
				fputs("\n| N Bit        |    N bit     |  Port      |",fptr_SYM);
				fputs("\n| of Variable  |   of Port    |  Name      |",fptr_SYM);
				fputs("\n--------------------------------------------",fptr_SYM);
				for (i=0; ; ) {
...
H_C_For_Proc() {
	...
	if (Current_For_Proc == (struct disc_forproc *)RW_LOCAL) {
		fputs("\n Visible from the proc ", fptr_SYM);
		fputs(Current_Process->name, fptr_SYM);
		fputs(" only (LOCAL).", fptr_SYM);
	}
	if (Current_For_Proc == (struct disc_forproc *)RW_ALL) {
		fputs("\n Visible from all processes (FOR ALL).", fptr_SYM);
	}
	if ((Current_For_Proc != (struct disc_forproc *)ZERO)
		...
		&& (Current_For_Proc != (struct disc_forproc *)RW_ALL)) {
			fputs("\n Visible from the following processes:\n\t", fptr_SYM);
			fputs(Current_Process->name, fptr_SYM);
			fputs("(Process-Owner)\n\t", fptr_SYM);
			for (;;) {
