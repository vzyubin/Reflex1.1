...
void hello (INT16S argc, char **argv){
	...
	Flag_CPP = OFF;					 /* Генерируем с-файл */
	Flag_NAMES = OFF;				 /* имена переменных конструируются из логических имен процессов, типа PiiVjj, */
									 /* альтернативно конструируются из имен процессов и имен переменных, типа <name>_<vfrname> */
	...
	if (argc == 1) {    /* NO PARAMETERS */
		printf("      ==================================================================\n");
		printf("      =                                                                =\n");
		printf("      =    Reflex V1.1, IA&E, Novosibirsk, Russia. Copyleft (c) 2019   =\n");
...
// NEW 
		...
		printf("      =        -CPP    (Create .cpp files)                             =\n");
		printf("      =        -PORT   (Create ports.h file)                           =\n");
		printf("      =        -T      (Test-mode generation)                          =\n");
		printf("      =        -CFG    (Create .cfg files)                             =\n");

// END NEW
// NEW 2019 
		printf("      =        -V      (Gen C-var-names from R-proc-var names)         =\n");
//		printf("      =        -DV     (Gen code for deductive verification)           =\n");
// END NEW
		...
		{   /* обработать блок опций командной строки */
			...
			case 'C':           /* касается ограничений С-транслятора */
				...
				case 'P':
					...
					else if (!strcmp(argv[i], "-CPP-")) Flag_CPP = OFF;
					else {
						printf("\n%%RCSL-64-FATERR, В командной строке обнаружен неизвестный ключ - %s", argv[i]);
						exit(ZERO);
					}
				...
				case 'F':
					...
					else if (!strcmp(argv[i], "-CFG-")) Flag_CONFIG = OFF;
					else {
						printf("\n%%RCSL-64-FATERR, В командной строке обнаружен неизвестный ключ - %s", argv[i]);
						exit(ZERO);
					}
...
//				break;
			...
			case 'P': /* необходимо создать файлы ports.h -PORT*/
				...
				else if (!strcmp(argv[i], "-PORT-")) Flag_PROC_DESCR = OFF;
				else {
					printf("\n%%RCSL-64-FATERR, В командной строке обнаружен неизвестный ключ - %s", argv[i]);
					exit(ZERO);
				}
...
//				break;
			...
			case 'V': /* генерация идентификаторов С-переменных из имен Reflex-процессов/переменных (только для английских текстов) */
				switch (argv[i][2]){
				case ZERO:
					Flag_NAMES = ON;      /* флаг имен */
#ifdef TEST_MODE
					printf("\n%%RCSL-64-MSG, флаг генерации C-имен из R-имен ВКЛЮЧЕН");
#endif
					break;
				case '-':
					Flag_NAMES = OFF;     /* флаг имен OFF */
					break;
				default:
					printf("\n%%RCSL-64-FATERR, В командной строке обнаружен неизвестный ключ - %s", argv[i]);
					exit(ZERO);
