...
int Sem_Statement_Body (void)
{
    ...
    switch (compare_RW(unit)) {
         ...
         case RW_START:
              ...
              break;
		 case RW_SET:			  /* 1.1 */
			 Sem_Set_Body();      /* SET (NEXT | STATE имя состояния) ; */
			 break;
		 case RW_RESET:			  /* 1.1 */
			 Sem_Reset_Body();    /* RESET  TIMEOUT ; */
			 break;
		 case RW_RESTART:			  /* 1.1 */
			 Sem_Restart_Body();    /* RESTART ; */
			 break;
...
void Sem_Reset_Body (void)
{
	...
	switch (compare_RW(unit)) {
	  case RW_TIMEOUT:            /* TIMEOUT */
		GenResetTimeoutForCurrentProc(); 
		select_unit();      /* ; */
		break;
	  default:                 /*  иначе - ошибка спецификации reset timeout */
...
*          ERROR (NONE) Нужно отматывать ч/з Find_Position
************************************************/
void Sem_Restart_Body (void)
{
#ifdef TEST_MODE_SEM
	printf("\n==== Sem_Restart_Body");
#endif
	GenSpaces();              /* формируем удобочитаемый текст */
	GenRestartForCurrentProc();
	select_unit();      /* ; */
	return;
};
 ...
 *  ВХОД - после обнаружения рез.слова УСТ (SET)
 *  unit  V
